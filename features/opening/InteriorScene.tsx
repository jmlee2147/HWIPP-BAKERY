 ...
 *
 * 배경·제빵사는 컷마다 갈아끼우지 않고 **전부 올려두고 불투명도만 바꾼다.** 이미지를
 * 교체하면 그 순간 디코딩이 일어나 TV SoC 에서 탭 반응이 밀린다. 대신 이 씬에 들어설 때
 * 한 번에 디코딩하므로 부팅 프리로드가 붙기 전까지는 컷 3→4 전환이 그 비용을 안는다
 * (`.claude/rules/assets.md` — 프리로드는 구현 순서 5단계다).
 */
